...
class GaussianMatrixSolverTest {
    /*
        2   1   0   0      1
        1   3   1   0      1
        0   1   3   1      1
    ...
    /*
        1   1   0      1  // first row fails diagonal dominance
        2   3   2      1
        0   2   1      1
     */
    ...
    double[] x4 = new double[]{0.5, 0, 0.5};



    double[][] m5 = new double[][]{
            {1, 1, 0,   0.5},
            {2, 2, 2,   0},
            {0, 1, 1,   0.5}
    };
    double[] x5 = new double[]{-0.5, 1, -0.5};
    ...
    void solve() {
        ...
        assertArrayEquals(x1, GaussianMatrixSolver.solve(PDESolverVTCS.toMatrix(a1, b1, c1, d1)), 1e-15);
        assertArrayEquals(x2, GaussianMatrixSolver.solve(PDESolverVTCS.toMatrix(a2, b2, c2, d2)), 1e-15);
        assertArrayEquals(x3, GaussianMatrixSolver.solve(PDESolverVTCS.toMatrix(a3, b3, c3, d3)), 1e-15);
        assertArrayEquals(x4, GaussianMatrixSolver.solve(PDESolverVTCS.toMatrix(a4, b4, c4, d4)), 1e-15);
        assertArrayEquals(x5, GaussianMatrixSolver.solve(m5), 1e-15);
        assertArrayEquals(x6, GaussianMatrixSolver.solve(m6), 1e-15);
